...
export default function RecipeDetail() {
  ...
  const affiliate = `https://www.amazon.com/s?k=${encodeURIComponent(recipe.ingredients.map(i => i.name).join(" "))}`;

  if (locked) {
    return (
      <div className="space-y-6">
        <Link to="/app/explore" className="inline-flex items-center gap-2 text-sm text-muted-foreground"><ArrowLeft className="size-4" /> Back</Link>
  ...
  return (
    <div className="space-y-8">
      <div className="relative -mx-4 sm:-mx-6 aspect-[4/3] sm:aspect-[16/9] overflow-hidden sm:rounded-2xl">
        ...
        <Link to="/app/explore" data-testid="recipe-back" className="absolute top-4 left-4 size-10 rounded-full glass grid place-items-center">
          <ArrowLeft className="size-4" />
        </Link>
        <button onClick={toggleSave} data-testid="recipe-save" className={`absolute top-4 right-4 size-10 rounded-full glass grid place-items-center ${saved ? "text-primary" : ""}`}>
      ...
      {/* Cooking video */}
      {recipe.video_url && (
        <section data-testid="recipe-video" className="nv-card nv-shadow overflow-hidden">
          {showVideo ? (
      ...
      {/* Nutrition label */}
      <section data-testid="nutrition-label" className="nv-card nv-shadow p-5">
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          ...
          <a href={affiliate} target="_blank" rel="noreferrer" data-testid="affiliate-link"
      ...
      <section className="nv-card nv-shadow p-5 space-y-3 sticky bottom-24 z-10">
        ...
        <div className="flex gap-3">
          <Select value={mealType} onValueChange={setMealType}>
            <SelectTrigger data-testid="log-meal-type" className="w-40"><SelectValue /></SelectTrigger>
            <SelectContent>
          ...
          <Button data-testid="log-meal-btn" onClick={logMeal} className="rounded-full flex-1"><Plus className="size-4 mr-1" /> Log meal</Button>
        </div>
      </section>
    </div>
  );
