...
export default function Track() {
  const { user } = useAuth();
  const [today, setToday] = useState({ totals: { calories: 0, protein: 0, carbs: 0, fat: 0 }, logs: [] });
  ...
  const pct = (cur, target) => Math.min(100, Math.round((cur / target) * 100)) || 0;
  ...
  return (
    <div className="space-y-8">
      ...
      {targets && (
        <section className="nv-card p-5 space-y-4" data-testid="track-targets">
          ...
          {[
            ...
            <div key={row.k} className="space-y-1.5">
              <div className="flex items-center justify-between text-sm">
                <span>{row.label}</span>
                <span className="text-muted-foreground">{today.totals[row.k]} / {row.t}</span>
              </div>
              <div className="h-2 w-full rounded-full bg-muted overflow-hidden">
                <div className="h-full rounded-full transition-all" style={{ width: `${pct(today.totals[row.k], row.t)}%`, background: row.color }} />
              </div>
            </div>
          ))}
        </section>
      ...
      <section>
        ...
        {today.logs.length === 0 ? (
          ...
          <ul className="space-y-2">
            {today.logs.map((l) => (
              <li key={l.id} className="nv-card px-4 py-3 flex items-center justify-between" data-testid={`log-item-${l.id}`}>
                <div>
                ...
                <button data-testid={`delete-log-${l.id}`} onClick={() => del(l.id)} className="size-8 grid place-items-center rounded-full hover:bg-muted">
