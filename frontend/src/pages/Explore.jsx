  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <Input data-testid="explore-search" placeholder="Try “idli sambar”, “Korean”, “PCOS”…"
          value={search} onChange={(e) => setSearch(e.target.value)}
          className="pl-9 rounded-full h-11 bg-card" />
      </div>

      <Tabs value={category} onValueChange={setCategory}>
        <TabsList className="grid grid-cols-5 w-full" data-testid="explore-tabs">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <Select value={country} onValueChange={setCountry}>
          <SelectTrigger data-testid="explore-country"><SelectValue placeholder="Country" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={region} onValueChange={setRegion} disabled={regions.length === 0}>
          <SelectTrigger data-testid="explore-region"><SelectValue placeholder="Region" /></SelectTrigger>
          <SelectContent>
        ...
        <div className="col-span-2 sm:col-span-1 flex gap-2 rounded-full border border-border p-1 bg-card">
          {[
            ...
            <button key={t.v} onClick={() => setTier(t.v)} data-testid={`tier-${t.v}`}
      ...
      <div className="flex gap-2 overflow-x-auto nv-scroll pb-2">
        <button onClick={() => setTag("")} data-testid="filter-chip-all"
        ...
        {TAGS.map((t) => (
          <button key={t} onClick={() => setTag(tag === t ? "" : t)} data-testid={`filter-chip-${t}`}
